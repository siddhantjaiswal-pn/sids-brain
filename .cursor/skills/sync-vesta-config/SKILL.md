...
3. Then fetch and write just that objective's detail:
   ...
   from sync_vesta_config import *
   obj = fetch_objective('{uuid}')
   ext_id = obj.get('externalIdentifier') or to_kebab(obj['name'])
   ...
   for t in obj.get('taskTemplates', []):
       s = write_task_file(t, tasks_dir, all_tasks_map)
       rows.append((t['name'], t['taskTemplateType'], t.get('entityType',''), t.get('triggerMethod',''), '—', '—', s))
   write_objective_readme(obj, obj_dir, rows)
